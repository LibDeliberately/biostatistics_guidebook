...
When we’ve already picked a test family, we still have to decide whether a **one-sided** or **two-sided** alternative hypothesis is the right choice. This is about the **direction** of $H_A$, not which test family to use (*t*-test, ANOVA, chi-square, and so on).
...
After we pick a test, we still have to state the alternative hypothesis. That alternative can look in **both directions** or in **only one**.

- **Two-sided** (also called two-tailed): we care about a difference in **either** direction. Example: mean BP on Drug A differs from mean BP on placebo, higher or lower.
- **One-sided** (also called one-tailed): we care about a difference in **only one** pre-specified direction. Example: mean BP on Drug A is **lower** than on placebo (we are not treating “higher” as support for our scientific claim in this test).
...
Most software and journals default to **two-sided** tests unless we clearly justify a one-sided alternative ahead of time.

---

## The One Question That Matters

I ask this **before looking at the results**:
...
If we find ourselves choosing the side after seeing which way the estimate went, that is a sign to stay two-sided.
...
**Tip:** “I expect the new treatment to be better” is not always enough by itself. Ask whether an effect in the **opposite** direction would still be scientifically important to detect and report. If yes, I’ll stick with two-sided.
...
3. I’ll choose a **one-sided** *t*-test (assuming that test’s other assumptions are met).  
4. Using alpha = 0.05, if significant, there is sufficient evidence that mean systolic BP is lower on the drug than on placebo. Report the mean difference and CI as well.  
5. If the drug’s mean is higher, we do **not** treat that as a significant finding under this one-sided $H_A$—we report the estimate and stay consistent with the pre-specified direction.

**Question B:** Do two diets differ in mean systolic BP?

1. Either higher or lower mean BP on Diet A vs Diet B would matter.  
2. I’ll choose a **two-sided** test.  
3. $H_0: \mu_A - \mu_B = 0$; $H_A: \mu_A - \mu_B \ne 0$.  
4. Using alpha = 0.05, if significant, there is sufficient evidence that the diet means differ; use the estimate to say which diet was higher.
